...
export const fetchVendorSuggestions = async (taskId: string): Promise<TaskSuggestionResponse> => {
  try {
    const response = await api.post('/api/vendors/by-task', { taskId });
    return response.data;
  } catch (error) {
...
export const fetchEnrichedSuggestions = async (eventName: string, eventDescription: string, venueNeeded: boolean, cateringNeeded: boolean) => {
  try {
    const response = await api.post('/api/cohere/suggest-tasks-enriched', {
      eventName,
      eventDescription,
      venueNeeded,
      cateringNeeded,
...
export const fetchVendorSuggestionsForTask = async (taskTitle: string, eventId: string) => {
  try {
    const response = await api.post('/api/vendors/by-task-and-event', { taskTitle, eventId });
    return response.data;
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : "Failed to fetch vendor suggestions for task.";
    toast.error(errorMessage);
    throw error;
  }
};
